...
description = (""" 
               ...
               """)
plt.figtext(0.02, 0.01, description, weight=400, wrap=True)
...
description = (""" 
               Principal components analysis did effectively retain 
               the distinct classes that the G1 data had originally.
               It is easy to see, even by eye, that the G2 data were 
               and still are in three distinct classes. However, 
               reducing the data to only 1-dimension makes it impossible
               to separate the blue class from the green. In this regard,
               PCA isn't favorable.
               """)
plt.figtext(0.02, 0.01, description, weight=400, wrap=True)
...
description = (""" 
               In this case, PCA doesn't help much to separate out the three
               classes of this G3 data set. The inherent structure of the
               data is well-preserved in that PCA reduced the dimensionality
               without collapsing the data into a narrower region than before.
               In other words, PCA simply takes a "bird's-eye view" of the
               data in this case. But it doesn't help much.
               """)
plt.figtext(0.02, 0.01, description, weight=400, wrap=True)
...
description = (""" 
               PCA again does a fine job at isolating the two data classes
               from each other in two dimensions, but it fails to reveal the 
               high variability between the red and green halfmoons in 
               dimension 3. It is almost no use to reduce the data to
               1-dimension in this case. 
               """)
plt.figtext(0.02, 0.01, description, weight=400, wrap=True)
...
description = (""" 
               PCA once again does a fine job at retaining the 
               inherent variability in the swissroll data set in 
               2-dimensions, but there's virtually no information
               to gain at the 1-dimensional reduction. Concerning the
               goal of this data set, it is true that farthest neighbors
               are still kept far from each other. However, nearest 
               neighbors blend in on top of one another. PCA doesn't reveal
               nearest neighbors!
               """)
plt.figtext(0.02, 0.01, description, weight=400, wrap=True)
...
description = (""" 
               The seeds collection of data looked like a complete mess
               in the 3-dim plot. However, in 2-dim, PCA revealed structure
               from the chaos! It is now easy to see the 3 distinct 
               classes.
               """)
plt.figtext(0.02, 0.01, description, weight=400, wrap=True)
